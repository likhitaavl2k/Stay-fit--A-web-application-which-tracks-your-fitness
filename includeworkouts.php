...
<div>
<p style='font-family:cursive;font-size:30px;' > Total available Workout recommendations= $total</p>
<table id='foodtable' >
<tr>
...
while($result=mysqli_fetch_assoc($data)){
  ...
  <img src='data:".$result['small_type'].";base64,".base64_encode($result['small_pic'])."' style='width:auto;height: 20%' class='center' </a></td> 
  <td><p style='max-width:100px;'> ".$result['name']." </p></td>
  <td><p style='max-width:350px;'>".$result['description']."</p></td>
  <td> ".$result['category']." </td>
  <td> ".$result['weight']." </td>
  
  <td><button type='button' class='btn btn-primary clickyButton' data-toggle='modal' data-id1='".$result['id']."' data-name='".$result['name']."' data-desc='".$result['description']."' data-weight='".$result['weight']."' data-category='".$result['category']."' data-target='#UpdateWorkoutPopup'  id='edit' style='padding:2px;'><img src='images/edit.svg' >Edit</button>
  ...
  <div class='modal fade' id='UpdateWorkoutPopup' tabindex='-1' role='dialog' aria-labelledby='exampleModalLabel'
  aria-hidden='true'>
  <div class='modal-dialog modal-xs' role='document'>
  <div class='modal-content'>
  <div class='modal-header' style='background-color: #5bc0de'>
  <h4 class='modal-title' id='myModalLabel'>Update workout Details</h4>
  <button type='button' class='close' data-dismiss='modal' aria-label='Close'>
  <span aria-hidden='true'>×</span>
  </button>
  </div>
  <form action='editworkout.php' method='POST' enctype='multipart/form-data'>
  ...
  <input type='text' class='form-control' name='name' id='workoutid'  style=' width: 90%'>
  </div>
  ...
  <label for='file2' style='color: black;position: absolute;left: 56%;top: 80%'><b>Upload GIF:</b></label>
  <input type='file' class='form-control-file' name='file2' style='position: absolute;left: 50%;top: 88%'>
  </div>
